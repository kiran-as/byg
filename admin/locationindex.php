...
$groundindexsql = mysql_query("Select * from tbl_location");
$i=0;
while($row = mysql_fetch_assoc($groundindexsql))
{
  $groundarray[$i]['status'] = $row['status'];
  $groundarray[$i]['locationname'] = $row['locationname'];
  $groundarray[$i]['idlocation'] = $row['idlocation'];
  $i++;
}
?>
<!DOCTYPE html>
<html lang="en">
  <head>
	...
	{
	   parent.location='locationadd.php';
	}
	</script>
  </head>
    ...
    <div class="row">
      <div class="form-group">        
        ...
        <div class="col-sm-4 txtr col-sm-offset-4">
            <button type="button" class="btn btn-primary mar-b5" onclick='fnaddground()';>+ Add New Location</button>
        </div>
      </div>               
    </div>    
...
?>
    ...
    <td><?php  echo $groundarray[$i]['locationname'];?></td>
		   <td><?php  echo $groundarray[$i]['status'];?></td>
		   <td><a href="locationedit.php?idlocation=<?php  echo $groundarray[$i]['idlocation'];?>">Edit</a></td>
          </tr>  
		  <?php }?>
